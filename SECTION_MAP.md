...
| `spatial/data/protected_lands_ca.gpkg` | 318 California protected-land polygons: 63 federal (USFWS NWRs, hatcheries, WMAs) + 255 state (CDFW Wildlife Areas, Ecological Reserves). Generated by `download_nwr.py`. | 3 MB |
...
**Refresh protected lands**: Run `python download_nwr.py` to re-pull from
USFWS (federal NWRs) and CDFW BIOS (state wildlife areas, ecological
reserves) and regenerate `protected_lands_ca.gpkg`. The map renders
USFWS units in sea-green and CDFW units in dodger-blue, with a checkbox
in the controls bar to toggle the layer on/off.
...
| `download_nwr.py` | One-time setup — downloads federal NWRs + CDFW state lands to `protected_lands_ca.gpkg` |
